...
/// Kiểm tra xem startup đã được bật chưa
    ...
    unsafe {
        ...
        match result {
            Ok(_) => {
                ...
                let reg_value = String::from_utf16_lossy(&buffer[..(buffer_size as usize / 2)]);

                // Remove null terminator
                let reg_value = reg_value.trim_end_matches('\0');

                Ok(reg_value == exe_path_str)
            }
            Err(err) => {
                if err.code() == ERROR_FILE_NOT_FOUND.to_hresult() {
                    Ok(false)
                    ...
                    Err(anyhow::anyhow!(
                        "Không thể kiểm tra registry value: {:?}",
                        err
                    ))
                }
            }
